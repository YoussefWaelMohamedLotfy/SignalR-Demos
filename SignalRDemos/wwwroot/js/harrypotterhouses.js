...
connectionHouse.on("subscriptionStatus", (strGroupsJoined, houseName, hasSubscribed) => {
    ...
    if (hasSubscribed) {
        ...
        showNewToast(`You have Subscribed Successfully in ${houseName}`);
    }
    else {
        //unsubscribe
        switch (houseName) {
        ...
        showNewToast(`You have Unsubscribed Successfully from ${houseName}`);
    }

});

connectionHouse.on("triggerHouseNotification", (houseName) => {
    showNewToast(`A new notification for ${houseName} has been launched.`);
});

connectionHouse.on("newMemberAddedToHouse", (houseName) => {
    showNewToast(`Member has subscribed to ${houseName}`);
});

connectionHouse.on("newMemberRemovedFromHouse", (houseName) => {
    showNewToast(`Member has unsubscribed from ${houseName}`);
});
...
const targetElement = document.querySelector('[data-toast-mode="stack"]');

targetElement.parentNode.removeChild(targetElement);

function showNewToast(message) {
    var newToast = targetElement.cloneNode(true);
    newToast.childNodes[3].innerText = message;
